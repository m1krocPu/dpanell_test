...
function alert(message, type="danger") {
    if ($(".alert-top-right").length <=0){
        $("body").append($("<div/>",{"class":"alert-top-right"}));

    }
    alertContainer = $(".alert-top-right");
    aid = Math.round(new Date().getTime() + (Math.random() * 100));

    alertContainer.append(
        $("<div/>",{"class":`alert  alert-${type} alert-dismissible`,"role":"alert","id":"alert"+aid})
            .append($("<span/>").html(message))
            .append(`<button class="btn-close" type="button" data-bs-dismiss="alert" aria-label="Close" data-bs-original-title="" title=""></button>`)
    );

    $count = $(".alert-top-right").find(".alert").length;


    $("#alert"+aid).fadeTo( (3000  * $count) - (2000 * ($count-1)), 500).slideUp(500);
    setTimeout(function() {
       $(".alert-top-right").find(".alert").first().remove();
    }, 3500  * $count);
